...
def mut_ref(df,loc,ref_file,complement,transcript,translate,table_def):
    chromosome=loc.split(":")[0]
    ref=read_reference.read_reference_sequence(ref_file,chromosome)
    coordinates=loc.split(":")[1]
    start=int(coordinates.split("-")[0])-1 # python 0-based
    end=int(coordinates.split("-")[1])-1 

    line3=""
    for i in range(0,df.shape[0]):
        if i==0:
            if start<int(df.loc[i,"Position"])-1: # python 0-based
                mut_seq=ref[start:int(df.loc[i,"Position"])-1] # python last index not include
            elif start==int(df.loc[i,"Position"])-1:
                mut_seq=""
            ...
            # SNV, MNV, indels happen to have same form
            mut_seq_T2=ref[start_Next:int(df.loc[i,"Position"])-1]
            mut_seq=mut_seq+mut_seq_T2          
    ...
    if complement=="1":
        rec3 = SeqRecord(Seq(mut_seq).complement(),id="Complement_Mut_seq",description="Complement of mutated sequence",)
        records.append(rec3)        
    
    # The actual biological transcription process works from the template strand, doing a reverse complement (TCAG CUGA) to give the mRNA. However, in Biopython and bioinformatics in general, we 
    # typically work directly with the coding strand because this means we can get the mRNA sequence just by switching T U.
    if transcript=="1":
