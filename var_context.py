	...
	for i in range(0,df.shape[0]):
		...
		if len(df.loc[i,"REF"]) > len(df.loc[i,"ALT"]):
			...
			if df.loc[i,"REF"].upper()[0]==df.loc[i,"ALT"].upper()[0]:
				S = df.loc[i,"5'_aligned"]-1-based
				E = df.loc[i,"3'_edge"]+1+1-based
				Ref_seq = Seq(ref[S:E].upper())
				Mut_seq = Seq(ref[S].upper())+" -"*(len(df.loc[i,"REF"])-1)+Seq(ref[df.loc[i,"5'_aligned"]+len(df.loc[i,"REF"])-1-based:E].upper())			
				...
				E = df.loc[i,"3'_edge"]+1+1-based
				Ref_seq = Seq(ref[S:E].upper())
				Mut_seq = Seq(ref[S].upper())+Seq(df.loc[i,"ALT"][0:].upper())+"-"*(len(df.loc[i,"REF"])-1)+Seq(ref[df.loc[i,"3'_edge"]+len(df.loc[i,"REF"])-1-based:E].upper())			
				REF_seq.append(str(Ref_seq))
				MUT_seq.append(str(Mut_seq))
		elif len(df.loc[i,"REF"]) < len(df.loc[i,"ALT"]):
			if df.loc[i,"REF"].upper()[0]==df.loc[i,"ALT"].upper()[0]:
				S = df.loc[i,"5'_aligned"]-based 
				E = df.loc[i,"3'_edge"]+1+1-based 
				Ref_seq = Seq(ref[S:E].upper())
			...
			E = df.loc[i,"3'_edge"]+1+1-based 
			Ref_seq = Seq(ref[S:E].upper())
			Mut_seq = Seq(ref[S].upper())+df.loc[i,"ALT"]+Seq(ref[df.loc[i,"5'_aligned"]+len(df.loc[i,"REF"])-based:E].upper())
			REF_seq.append(str(Ref_seq))
			MUT_seq.append(str(Mut_seq))
