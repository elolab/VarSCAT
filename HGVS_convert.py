						...
						if out_seq_RT==0:
							...
							if len(in_seq_RM)==1:
								...
								if (out_seq_RT+in_seq_RT)==2: 
									...
									item = str(df.loc[i,"Chromosome"])+":"+g_prefix+"."+str(out_seq_left)+in_seq_RM+"["+str(out_seq_RT+in_seq_RT)+"]"		

		elif df.loc[i,"REF"].upper()[0]!=df.loc[i,"ALT"].upper()[0]:
			if len(df.loc[i,"REF"])==1 and len(df.loc[i,"ALT"])==1:
				item = str(df.loc[i,"Chromosome"])+":"+g_prefix+"."+str(df.loc[i,"Position"])+df.loc[i,"REF"]+">"+df.loc[i,"ALT"]
			elif len(df.loc[i,"REF"])!=1 or len(df.loc[i,"ALT"])!=1:	 			
				if len(df.loc[i,"REF"])==1:
					item = str(df.loc[i,"Chromosome"])+":"+g_prefix+"."+str(df.loc[i,"Position"])+"delins"+df.loc[i,"ALT"]
				elif len(df.loc[i,"REF"])!=1 and len(df.loc[i,"REF"])!=len(df.loc[i,"ALT"]):
					item = str(df.loc[i,"Chromosome"])+":"+g_prefix+"."+str(df.loc[i,"Position"])+"_"+str(int(df.loc[i,"Position"])+len(df.loc[i,"REF"])-1)+"delins"+df.loc[i,"ALT"]
				elif len(df.loc[i,"REF"])!=1 and len(df.loc[i,"REF"])==len(df.loc[i,"ALT"]):
					if Seq(df.loc[i,"REF"]).reverse_complement()==df.loc[i,"ALT"]:
						item = str(df.loc[i,"Chromosome"])+":"+g_prefix+"."+str(df.loc[i,"Position"])+"_"+str(int(df.loc[i,"Position"])+len(df.loc[i,"REF"])-1)+"inv"
					else:
						item = str(df.loc[i,"Chromosome"])+":"+g_prefix+"."+str(df.loc[i,"Position"])+"_"+str(int(df.loc[i,"Position"])+len(df.loc[i,"REF"])-1)+"delins"+df.loc[i,"ALT"]
