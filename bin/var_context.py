	...
	for i in range(0,len(df)):
		...
		if len(df[i][2]) > len(df[i][3]):
			...
			if df[i][2].upper()[0]==df[i][3].upper()[0]:
				S = df[i][len_header]-1-based
				E = df[i][len_header+2]+1+1-based
				Ref_seq = ref[S:E].upper()
				Mut_seq = ref[S].upper()+" -"*(len(df[i][2])-1)+ref[df[i][len_header]+len(df[i][2])-1-based:E].upper()	
				...
				E = df[i][len_header+2]+1+1-based
				Ref_seq = ref[S:E].upper()
				Mut_seq = ref[S].upper()+df[i][3][0:].upper()+"-"*(len(df[i][2])-1)+ref[df[i][len_header+2]+len(df[i][2])-1-based:E].upper()			
				REF_seq.append(str(Ref_seq))
				MUT_seq.append(str(Mut_seq))
		elif len(df[i][2]) < len(df[i][3]):
			if df[i][2].upper()[0]==df[i][3].upper()[0]:
				S = df[i][len_header]-based 
				E = df[i][len_header+2]+1+1-based 
				Ref_seq = ref[S:E].upper()
			...
			E = df[i][len_header+2]+1+1-based 
			Ref_seq = ref[S:E].upper()
			Mut_seq = ref[S].upper()+df[i][3]+ref[df[i][len_header]+len(df[i][2])-based:E].upper()
			REF_seq.append(str(Ref_seq))
			MUT_seq.append(str(Mut_seq))
