...
def HGVS_transform(df,ref_file,based,len_header):
	HGVS_result=[]
	chr_name=""
	for i in range(0,len(df)):
		...
		if df[i][2].upper()[0]==df[i][3].upper()[0]:
			if len(df[i][2])>len(df[i][3]):
				if int(df[i][len_header+1])==int(df[i][1])+1:
					if len(df[i][2])-len(df[i][3])==1:
						item = str(df[i][0])+":"+g_prefix+"."+str(df[i][len_header+1])+"del"
					elif len(df[i][2])-len(df[i][3])>1:
						item = str(df[i][0])+":"+g_prefix+"."+str(df[i][len_header+1])+"_"+str(int(df[i][len_header+1])+len(df[i][2])-len(df[i][3])-1)+"del"
				elif int(df[i][len_header+1])!=int(df[i][1])+1:
					if len(df[i][2])-len(df[i][3])>1:
						Right_align_seq=ref[int(df[i][len_header+1])-based:int(df[i][len_header+2])-based+1].upper()
						in_seq_result=in_seq_pattern(Right_align_seq)
						in_seq_RT=in_seq_result[0]
						in_seq_RM=in_seq_result[1]
						
						out_seq_result=out_seq_pattern(in_seq_RM,ref,based,int(df[i][len_header+1]))
				...
				if int(df[i][len_header+1])==int(df[i][1]):
					item = str(df[i][0])+":"+g_prefix+"."+str(df[i][len_header+1])+"_"+str(df[i][len_header+1]+1)+"ins"+str(df[i][3][1:])
				elif int(df[i][len_header+1])!=int(df[i][1]):
					if len(df[i][3])-len(df[i][2])==1:
						item = str(df[i][0])+":"+g_prefix+"."+str(df[i][len_header+1])+"dup"
					elif len(df[i][3])-len(df[i][2])>1:
						Right_align_seq=form_shifted_variant(df[i][3][1:],int(df[i][len_header+1]),int(df[i][1]))
						in_seq_result=in_seq_pattern(Right_align_seq)
						in_seq_RT=in_seq_result[0]
						in_seq_RM=in_seq_result[1]
						
						out_seq_result=out_seq_pattern(in_seq_RM,ref,based,int(df[i][len_header+1])+1) 
						out_seq_left=out_seq_result[0]
						out_seq_right=out_seq_result[1]
						out_seq_RT=out_seq_result[2]
						if out_seq_RT==0:
							item = str(df[i][0])+":"+g_prefix+"."+str(df[i][len_header+1])+"_"+str(df[i][len_header+1]+1)+"ins"+Right_align_seq
						elif out_seq_RT!=0:
							if len(in_seq_RM)==1:
								if out_seq_RT<len(df[i][3][1:]):
									item = str(df[i][0])+":"+g_prefix+"."+str(df[i][len_header+1])+"_"+str(df[i][len_header+1]+1)+"ins"+str(df[i][3][1:])
								elif out_seq_RT>=len(df[i][3][1:]):
									item = str(df[i][0])+":"+g_prefix+"."+str(df[i][len_header+1]-len(df[i][3])+1+1)+"_"+str(df[i][len_header+1])+"dup"
							elif len(in_seq_RM)>1:
